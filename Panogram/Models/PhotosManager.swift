...
class PhotosManager {
  ...
  func fetchImage(at index: Int, cached: Bool, completion:@escaping (Int, UIImage) -> Void) {
    ...
    queue.async { [row = index] in
      if cached == true {
        let requiredImageSize = CGSize(width: 600, height: 200)
        PHImageManager.default()
          .requestImage(for: asset, targetSize: requiredImageSize,
                        contentMode: .default, options: options,
                        resultHandler: { (image, info) in
                          guard let infoDict = info as? [String: Any] else {return}
                          if let isDegraded = infoDict[PHImageResultIsDegradedKey] as? Bool,
                            isDegraded == false {
